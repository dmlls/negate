...
class Negator(BaseNegator):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if use_transformers and use_gpu:
            ...
            spacy.require_cpu()
        self.spacy_model = self._initialize_spacy_model(
            model_name="en_core_web_trf" if use_transformers else "en_core_web_md",
            model_version=(
                EN_CORE_WEB_TRF_VERSION if use_transformers else EN_CORE_WEB_MD_VERSION
            ),
            **kwargs,
        )
    ...
    def negate_sentence(
        ...
        **kwargs: Dict[str, Any],
        ...
        if negation:
            ...
            if (
                not self._is_aux(root)
                and root.tag_ not in ("VBN", "VBG")
                and not aux_child
                and not self._is_verb_to_do(root)
            ...
            # Any other AUX or verb in past participle -> Remove negation.
                ...
                if contains_inversion:  # E.g.: "Does she not know about it?"
                    ...
                    if negation.i < root.i and negation.i > 0:
                        doc[negation.i - 1]._.has_space_after = (
                            negation._.has_space_after
                        )
                    # Correctly handle space in e.g., "I'm not doing great." vs.
                    # "I am not doing great."
                    space_before = " " * int(
                        root.i > 0 and doc[root.i - 1]._.has_space_after
                    )
            ...
            return self._compile_sentence(doc, remove_tokens=remove, add_tokens=add)

        # AUX as ROOT (e.g.: "I'm excited.") or ROOT children e.g.,
        # "I do think...".
        if (
            self._is_aux(root)
            or self._is_verb_to_be(root)
        ...
        if any(child.tag_ == "TO" for child in root.children) or root.tag_ == "VBG":
            # E.g.: "A Python module negating sentences." or "A Python module
            # to negate sentences."
            add = f"not {root.text}"
        else:
            negated_aux = self.negate_aux(
                self.conjugate_verb("do", root.tag_), prefer_contractions
            )
        ...
        return self._compile_sentence(
            ...
            add_tokens={
                root.i: Token(text=add, has_space_after=root._.has_space_after)
            },
        )
    ...
    def _negate_aux_in_doc(
        ...
        if negation:
            ...
            if not remove and not add:
                ...
                add = Token(text=aux.text, has_space_after=negation._.has_space_after)
            return self._compile_sentence(
                doc, remove_tokens=remove, add_tokens={aux.i: add}
            )
        ...
        # version.
        if contains_inversion and (aux.text.lower() == "am" or not prefer_contractions):
            ...
            if aux.i + 1 < len(doc) and doc[aux.i + 1].text.lower() == "'ve":
                ...
                add[aux.i + 1] = Token(
                    text=" have", has_space_after=doc[aux.i + 1]._.has_space_after
                )
        return self._compile_sentence(doc, remove_tokens=remove, add_tokens=add)

    def _handle_ca_wo_sha(
        self, *aux_tokens: Optional[SpacyToken], negation: SpacyToken
        ...
        for aux in aux_tokens:
            ...
            if aux.text.lower() == "wo":
                ...
                add.update(
                    {
                        aux.i: Token(
                            text=" will", has_space_after=negation._.has_space_after
                        )
                    }
                )
            # Case AUX "can't" -> Remove negation and replace "ca" -> "can".
                ...
                add.update(
                    {
                        aux.i: Token(
                            text=" can", has_space_after=negation._.has_space_after
                        )
                    }
                )
            # Case AUX "shan't" -> Remove negation and replace "sha" -> "shall".
                ...
                add.update(
                    {
                        aux.i: Token(
                            text=" shall", has_space_after=negation._.has_space_after
                        )
                    }
                )
            if remove and add:
    ...
    def _parse(self, string_: str) -> SpacyDoc:
        ...
        for tk in doc:
            has_space_after: bool = i + len(tk) < len(string_) and (
                string_[i + len(tk)] == " "
            )
    ...
    def _get_entry_point(
        self, doc: SpacyDoc, contains_inversion: bool
    ) -> Optional[SpacyToken]:
        ...
        if not (self._is_aux(root) or self._is_verb(root)):
            ...
            if root.children:
                entry_point = [
                    tk for tk in root.children if self._is_aux(tk) or self._is_verb(tk)
                ]
            # No AUX or VERB found in the root children -> Take the first
            # AUX or VERB in the sentence, if any.
            if not entry_point:
                entry_point = [
                    tk for tk in doc if self._is_aux(tk) or self._is_verb(tk)
                ]
    ...
    def _get_first_negation_particle(self, doc: SpacyDoc) -> Optional[SpacyToken]:
    ...
    def _get_negated_child(
        self, token: SpacyToken, min_index: int = 0
    ) -> Optional[SpacyToken]:
        ...
        child = [
            ...
            if child.dep == neg and child.i >= min_index
        ]
    ...
    def _get_aux_child(
        ...
        child = [
            child
            for child in token.children
            if self._is_aux(child) and child.i >= min_index
        ]
        return child[0] if child else None

    def _get_first_aux_or_verb(self, doc: SpacyDoc) -> Optional[SpacyToken]:
    ...
    def _get_parent(self, token: SpacyToken, doc: SpacyDoc) -> Optional[SpacyToken]:
    ...
    def _contains_inversion(self, doc: SpacyDoc) -> bool:
        ...
        for tk in doc:
            ...
            if self._is_pronoun(tk) and not self._is_noun(self._get_parent(tk, doc)):
                pronoun = tk
            if aux and pronoun:
                break
        else:
